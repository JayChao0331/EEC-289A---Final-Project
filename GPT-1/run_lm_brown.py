...
def train(model, criterion, optimizer, train_loader, valid_loader, device):
    ...
    for epoch in range(num_epochs):
        ...
        if valid_loss <= min_loss:
            ...
            min_loss = valid_loss
            if pretrained:
                torch.save(model.state_dict(), '{}gram_brown_pretrained_gpt_model.pth'.format(N))
            else:
                torch.save(model.state_dict(), '{}gram_brown_without_pretrained_gpt_model.pth'.format(N))
...
if __name__ == '__main__':
    ...
    train_set, test_set = train_test_split(corpus_tokenized, ratio=0.8)
    ...
    model_path = None
    if pretrained:
        model_path = '{}gram_brown_pretrained_gpt_model.pth'.format(N)
    else:
        model_path = '{}gram_brown_without_pretrained_gpt_model.pth'.format(N)
